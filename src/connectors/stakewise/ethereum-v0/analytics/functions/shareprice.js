...
async function checkStakewiseSharePrice(
  chain,
  poolAddress,
  distributorAddress,
) {
  try {
    ...
    const shareAprIncrease = (await RETH2.rewardPerToken()) / 10 ** 18;
    const shareApr = 1 + shareAprIncrease;

    const sharePrice = totalAssets / totalSupply;
    return {
      data: { sharePrice: sharePrice, shareApr: shareApr },
      err: null,
    };
  } catch (err) {
    console.log(err);
