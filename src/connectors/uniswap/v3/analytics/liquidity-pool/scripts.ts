...
import { CollectOptions, NonfungiblePositionManager, Pool as UNISWAP_POOL, Position, RemoveLiquidityOptions } from "@uniswap/v3-sdk"
...
export async function getRemoveLiquidityMinimumAmounts(
  ...
  const token0 = new Token(await getChainId(chain), poolInfo.token0, await erc20Decimals(provider, poolInfo.token0));
  const token1 = new Token(await getChainId(chain), poolInfo.token1, await erc20Decimals(provider, poolInfo.token1));
  ...
  const multiCallData = NonfungiblePositionManager.INTERFACE.decodeFunctionData("multicall", calldata);
  const decreaseData = NonfungiblePositionManager.INTERFACE.decodeFunctionData("decreaseLiquidity", multiCallData.data[0]);
  return {
...
export async function constructPosition(
  ...
  const NFTManager = new ethers.Contract(pool.investing_address, NonfungiblePositionManager.INTERFACE, provider);
...
export async function getPoolInfo(chain: string, poolAddress: string): Promise<PoolInfo> {
  const provider = getNodeProvider(chain)
...
getRemoveLiquidityMinimumAmounts("ethereum", {
    ...
    }, ethers.constants.AddressZero, "1").then(response => console.log(response))