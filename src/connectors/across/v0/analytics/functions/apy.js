...
async function checkAcrossV0APY(chain, poolAddress) {
  try {
    ...
    const rewardTokenDecimals = await erc20Decimals(
      provider,
      poolInfo.rewards_tokens[0]
    );
    const underlyingTokenDecimals = await erc20Decimals(
      provider,
      poolInfo.underlying_tokens[0]
    );
    ...
    const cumulativeStakedUsd =
      (overallStaking.cumulativeStaked *
        underlyingTokenPrice *
        exchangeRateCurrent) /
      10 ** underlyingTokenDecimals;
    const rewardsPerYearUsd =
      (overallStaking.baseEmissionRate * rewardTokenPrice * SECONDS_PER_YEAR) /
      10 ** rewardTokenDecimals;
    const rewardApy = rewardsPerYearUsd / cumulativeStakedUsd;

    return {
      data: { activityApy: estimatedApy, rewardApy: rewardApy },
      err: null,
    };
  } catch (err) {
    console.log(err);
