...
async function analytics (
  ...
  try {
    ...
    const underlyingToken = new ethers.Contract(poolInfo.underlying_tokens[0], ERC20ABI, provider)


    const Pools = await axios.get(URL)
    const filteredPool = Pools.data.filter(pool => pool.pool === poolInfo.name);
    ...
    const share_price = (totalAsset/(10**decimalsUnderlyingToken))/(totalSupplyBN/(10*decimalsPoolToken))
