...
async function analytics (chain,poolAddress) {
  ...
  const rateToAPY = (ratePerYear) => Math.pow(1 + ratePerYear / SECONDS_PER_YEAR, SECONDS_PER_YEAR) - 1;
  ...
  const currentPoolInfo = allPoolInfo.find(
    market => market.address.toLowerCase() == poolAddress.toLowerCase());
  ...
  const totalSupplyOnPool =
      (+currentPoolInfo.metrics.supplyBalanceOnPool *
        +currentPoolInfo.reserveData.supplyPoolIndex) / (10 ** (27 + currentPoolInfo.token.decimals)) ;
  

   // supplied amount which is matched p2p
   const totalSupplyP2P =
   (+currentPoolInfo.metrics.supplyBalanceInP2P *
     +currentPoolInfo.p2pData.p2pSupplyIndex) / (10 ** (27 + currentPoolInfo.token.decimals)) ;
   

 // borrowed amount from underlying aave pool
 const totalBorrowOnPool =
   (+currentPoolInfo.metrics.borrowBalanceOnPool *
     +currentPoolInfo.reserveData.borrowPoolIndex) / (10 ** (27 + currentPoolInfo.token.decimals)) ;
   

 // borrowed amount which is matched p2p
 const totalBorrowP2P =
   (+currentPoolInfo.metrics.borrowBalanceInP2P *
     +currentPoolInfo.p2pData.p2pBorrowIndex) / (10 ** (27 + currentPoolInfo.token.decimals)) ;
 ...
 // in morpho's case we use total supply as tvl instead of available liq (cause borrow on morpho can be greater than supply (delta is routed to underlying aave pool)
    const totalSupplyUsd =
      tokenPrice * ((totalSupply) * (currentPoolInfo.reserveData.eth / 1e18)) / ethPrice;
    const totalBorrowUsd =
      tokenPrice * ((totalBorrow) * (currentPoolInfo.reserveData.eth / 1e18)) / ethPrice;
