...
async function analytics(chain, poolAddress) {
  ...
  const compMarket = allPoolInfo.find(
    (market) => market.token.address === compToken,
  );
  ...
  const totalSupplyOnPool =
    ...
    10 ** (18 + currentPoolInfo.token.decimals);
  ...
  const totalSupplyUsd =
    totalSupply *
    (currentPoolInfo.reserveData.usd /
      10 ** (18 * 2 - currentPoolInfo.token.decimals));
