...
const priceCoin = async (coin) => {
  const response_ = await axios.get(`${PRICE_API}${coin}`);
  const data_ = response_.data;
  return data_.coins[`ethereum:${coin}`].price;
};

const curveApyData = async () => {
  const respose_ = await axios.get(CURVE_APY_API);
  const data_ = respose_.data;
  return data_.apys;
};

const curvePoolData = async () => {
  const response_ = await axios.get(CURVE_POOL_API);
  const data_ = response_.data;
