...
async function checkAaveV3OtherAPYs (chain, poolAddress){
  try {
    const SUBGRAPH_URL = SUBGRAPH_URLS[chain]
    const { markets } = await request(SUBGRAPH_URL, reserveQueryOthers)
    const data = await markets.filter(
...
async function checkAaveV3APYs (chain, poolAddress, totalSupplyUSD) { 
  ...
  let rewards;
  const validChains = ["avalanche", "fantom", "polygon", "arbitrum", "optimism"];
