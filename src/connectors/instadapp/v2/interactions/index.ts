...
async function redeem(
  ...
  const amountStethToReceive = parseFloat(amount.amount) * exchangePrice;
  const amountBN = await toBnERC20Decimals(
    amount.amount,
    pool.chain,
    position_token
  );
  const amountStethBN = await toBnERC20Decimals(
    String(amountStethToReceive),
    pool.chain,
    '0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84'
  );
  const INSTADAPP_WRAPPER_API = `https://api.instadapp.io/defi/mainnet/1inch/v5/swap?buyToken=0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE&sellToken=0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84&sellAmount=${amountStethBN.toString()}&dsaAddress=0x0000000000000000000000000000000000000000`;
  const { data } = await axios.get(INSTADAPP_WRAPPER_API);
  const swapCalldata = data.calldata;
  const outTokenAmount = data.buyTokenAmount;
