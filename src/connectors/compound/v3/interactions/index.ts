...
async function deposit(
  ...
  const abi = PoolABI;
  const method_name = 'supply(address,uint256)';
...
async function redeem(
  ...
  const abi = PoolABI;
  const method_name = 'withdraw(address,uint256)';
  const amountBN = await toBnERC20Decimals(
    amount.amount,
    pool.chain,
    pool.pool_address
...
async function claimRewards(
  ...
  const method_name = 'claim(address,address,bool)';
  const args = [pool.pool_address,addresses.userAddress, true];
