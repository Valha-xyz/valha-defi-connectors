...
const calcRewardApy = (rewards, TVL, COMP_PRICE) => {
  return (
    ((rewards *BLOCKS_PER_DAY *365*COMP_PRICE) / TVL) *
    100
  );
};
...
async function analytics(chain, poolAddress) {
  ...
  const COMP_PRICE = price2 ? price2 : 20; // Hardwrite asset price

  const reward = await checkCompoundV2Rewards(chain, poolAddress);
  const RewAPY = reward.data.apyRewards ? calcRewardApy(reward.data.apyRewards, TVL, COMP_PRICE) : 0;
