...
exports.tvl = async (dataNow, networkString) => {
  ...
  for (const el of dataNowCopy) {
    const price0 = prices[`${networkString}:${el.token0.id}`]?.price;
    const price1 = prices[`${networkString}:${el.token1.id}`]?.price;
...
exports.apy = (entry, dataPrior, version) => {
  ...
  entry.volumeUSDPrior = dataPrior.find((el) => el.id === entry.id)?.volumeUSD;

  // calc 24h volume
  entry.volumeUSD24h = Number(entry.volumeUSD) - Number(entry.volumeUSDPrior);
